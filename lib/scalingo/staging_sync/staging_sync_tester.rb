    ...
    class StagingSyncTester
      ...
      def test_configuration_file
        ...
        if File.exist?(@config_file)
          ...
          fail "scalingo_staging_sync.yml not found at #{@config_file}"
      ...
      def test_security_guards
        ...
        if ENV["APP"].present?
          ...
          info "  APP: not set"
          @logger.logger.debug "[StagingSyncTester] APP environment variable not set"
        end
      end
      ...
      def test_slack_integration
        ...
        if defined?(Scalingo::StagingSync::SlackNotificationService)
          ...
          missing_methods = required_methods.reject { |m| Scalingo::StagingSync::SlackNotificationService.new.respond_to?(m) }
